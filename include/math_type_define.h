...
namespace DyrosMath
{
		...
		const Eigen::Matrix3d &rotation_f)
	{
		...
		double tau = cubic(time, time_0, time_f, 0, 1, 0, 0);
		double tauDot = cubicDot(time, time_0, time_f, 0, 1, 0, 0, 100);
		Eigen::Matrix3d rot_scaler_skew;
		rot_scaler_skew = (rotation_0.transpose() * rotation_f).log();
		//rot_scaler_skew = rot_scaler_skew.log();
		/*
...
/*		Eigen::Matrix3d result = (rot_scaler_skew).exp();
		Eigen::Vector3d result2;
		result2(0) = result(7) * tauDot;
		result2(1) = result(2) * tauDot;
		result2(2) = result(3) * tauDot;
		result2 = rotation_0 * result2;	*/	
		//Eigen::Matrix3d result = (rot_scaler_skew * tau).exp() / tau;
		Eigen::Matrix3d result = (rot_scaler_skew).exp();

		Eigen::Vector3d result2;
		result2(0) = result(2,1);
		result2(1) = result(0,2);
		result2(2) = result(1,0);
		double reault2_unit = sqrt(result2(0)*result2(0) + result2(1)*result2(1) + result2(2)*result2(2));
		result2 = result2 / reault2_unit;
		result2 = rotation_0 * result2 * tauDot;
...

	{
		...
		R_temp << x_f - x_0 - x_dot_0 * time_s - x_ddot_0 *
			pow(time_s, 2) /
			2.0,
